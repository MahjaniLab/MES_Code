...
# Desription: Testing comorbidities for MES gene carriers without ASD
...
for (gene in 1:length(GENES$V1)){
  ...
  # Removes individuals with a MES variant in another gene
  # Comment out to avoid
  ...
  for (column in QUES$V1){
    ...
    matrix_chr[1,2] = length(PHENO_sub[NAMES %in% NON_ASD_W_GENE]) - sum(PHENO_sub[NAMES %in% NON_ASD_W_GENE] == 1, na.rm = TRUE)
    matrix_chr[2,1] = sum(PHENO_sub[NAMES %in% NON_ASD_WO_GENE] == 1, na.rm = TRUE)
    matrix_chr[2,2] = length(PHENO_sub[NAMES %in% NON_ASD_WO_GENE]) - sum(PHENO_sub[NAMES %in% NON_ASD_WO_GENE] == 1, na.rm = TRUE)
